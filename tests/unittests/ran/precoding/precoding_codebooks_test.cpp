...
TEST(precoding_codebooks_test, TypeISinglePanelMode1_UnsupportedCases)
{
  ...
  // Select a Type I Single-Panel configuration corresponding to N1 = 4, N2 = 2, which is unsupported.
  pmi_codebook_single_panel_config panel_config = pmi_codebook_single_panel_config::four_two;

  // Create an example PMI.
  precoding_matrix_indicator pmi =
      pmi_typeI_single_panel{.panel_config = panel_config, .i_1_1 = 0, .i_1_2 = 0, .i_1_3 = std::nullopt, .i_2 = 0};

  // Check that an assert is triggered, and it is actually the one it is expected.
  ASSERT_DEATH(make_type1_sp_mode1(pmi, 4),
               R"(The given number of ports \(i\.e\., 16\) is out of the range of supported values \(1\.\.8\).)");

  // Set a supported panel config.
  panel_config = pmi_codebook_single_panel_config::two_one;
  pmi = pmi_typeI_single_panel{.panel_config = panel_config, .i_1_1 = 0, .i_1_2 = 0, .i_1_3 = std::nullopt, .i_2 = 0};
  ...
  ASSERT_DEATH(
      make_type1_sp_mode1(pmi, 8),
      R"(The given number of layers \(i\.e\., 8\) for the given number of ports \(i\.e\., 4) is out of the range of supported values \(1\.\.4\).)");

  // Set a 2D panel configuration.
  panel_config = pmi_codebook_single_panel_config::two_two;
  // Leave the vertical beam selector (i_1_2) empty.
  pmi = pmi_typeI_single_panel{
      .panel_config = panel_config, .i_1_1 = 0, .i_1_2 = std::nullopt, .i_1_3 = std::nullopt, .i_2 = 0};

  // Try to generate a precoding without the required i_1_2. Check that an assert is triggered, and it is actually the
  // one expected.
  ASSERT_DEATH(make_type1_sp_mode1(pmi, 5),
  ...
  // zero value.
  panel_config = pmi_codebook_single_panel_config::two_one;
  pmi = pmi_typeI_single_panel{.panel_config = panel_config, .i_1_1 = 0, .i_1_2 = std::nullopt, .i_1_3 = 1, .i_2 = 0};

  // Check that an assert is triggered, and it is actually the one expected.
  ASSERT_DEATH(
      make_type1_sp_mode1(pmi, 3),
