...
public:
  ...
  /// Construct orbit ephemeris information from a state vector.
  ...
  explicit orbit_ephemeris_info(orbital_propagation_model& propagator,
                                time_point                 ref_time,
                                const state_vector&        state,
                                bool                       eci_ref_frame,
                                std::optional<time_point>  current_time = std::nullopt);

  /// Construct orbit ephemeris information from orbital elements.
  /// \param propagator   Reference to the orbital propagation model to be used for orbit propagation.
  /// \param ref_time     Reference time when ECEF and ECI frames are aligned.
  ...
  /// Get the reference time when ECEF and ECI frames are aligned.
  /// \return const reference to the reference time.
  const time_point& reference_time() const { return ref_time_val; }

  /// Get the current epoch time associated with the orbit state.
  /// \return const reference to the epoch time.
  const time_point& epoch_time() const { return epoch_time_val; }
