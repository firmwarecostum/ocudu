...
expected<orbit_ephemeris_info*> ntn_assistance_info_generator::get_ephemeris_info(time_point t)
{
...
expected<const ntn_gateway_location_info*> ntn_assistance_info_generator::get_ntn_gateway_location_info(time_point t)
{
...
sib19_ntn_configs_reply ntn_assistance_info_generator::generate_ntn_config(const sib19_ntn_configs_request& req)
{
  ...
  expected<orbit_ephemeris_info*> ephemeris_info_exp = get_ephemeris_info(req.epoch_time);
  if (!ephemeris_info_exp.has_value()) {
    return reply;
  }

  std::optional<const ntn_gateway_location_info*> ntn_gateway_location;
  if (req.feeder_link_present) {
    expected<const ntn_gateway_location_info*> ntn_gateway_loc_exp = get_ntn_gateway_location_info(req.epoch_time);
    if (!ntn_gateway_loc_exp.has_value()) {
      return reply;
    }
    ntn_gateway_location = *ntn_gateway_loc_exp;
  }

  // Ephemeris info available.
  orbit_ephemeris_info* ephemeris_info = *ephemeris_info_exp;
  ...
  if (ntn_gateway_location.has_value()) {
