...
void xnap_impl::handle_cho_cancel_required(cu_cp_ue_index_t ue_index, const nr_cell_global_id_t& target_cgi)
{
  ...
  if (peer_ue_id == peer_xnap_ue_id_t::invalid) {
    ...
    ho_cancel->cause.set_radio_network()      = cause_radio_network_layer_opts::unspecified;
...
void xnap_impl::handle_handover_success_required(cu_cp_ue_index_t ue_index, const nr_cell_global_id_t& cgi)
{
  ...
  ue_ctxt.logger.log_info("Sent HandoverSuccess");
}
...
{
  // HandoverSuccess is sent from target -> source. From the source's perspective,
  // source_ng_ra_nnode_ue_xn_ap_id is our own (local) XNAP UE ID set in the HandoverRequest.
  local_xnap_ue_id_t local_xnap_ue_id = uint_to_local_xnap_ue_id(msg->source_ng_ra_nnode_ue_xn_ap_id);
  if (!ue_ctxt_list.contains(local_xnap_ue_id)) {
  ...
  // target_ng_ra_nnode_ue_xn_ap_id is the target's local XNAP UE ID, stored at the source as peer_xnap_ue_id.
...
void xnap_impl::handle_conditional_ho_cancel(const asn1::xnap::conditional_ho_cancel_s& msg)
{
  ...
  // source_ng_ra_nnode_ue_xn_ap_id = our LOCAL XNAP UE ID (we allocated this when we initiated HO).
  // target_ng_ra_nnode_ue_xn_ap_id = the cancelling target's UE ID (our peer_xnap_ue_id).
